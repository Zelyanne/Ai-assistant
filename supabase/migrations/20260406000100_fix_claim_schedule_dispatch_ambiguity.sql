-- Migration: Fix claim_schedule_dispatch schedule_id ambiguity
...
BEGIN
  ...
  -- Defensive input validation: the scheduler must provide a sane window.
  IF dispatch_window_start IS NULL OR dispatch_window_end IS NULL THEN
    RAISE EXCEPTION 'claim_schedule_dispatch requires non-null dispatch_window_start and dispatch_window_end';
  END IF;

  IF dispatch_window_end <= dispatch_window_start THEN
    RAISE EXCEPTION 'Invalid dispatch window: dispatch_window_end must be after dispatch_window_start';
  END IF;
  ...
  -- Caller must not dispatch windows before the schedule is due.
  IF dispatch_window_start < schedule_row.next_run THEN
    RAISE EXCEPTION 'Invalid dispatch_window_start: before schedule next_run';
  END IF;
  ...
  -- Avoid PL/pgSQL parameter/column name collisions (e.g., schedule_id).
...
COMMENT ON FUNCTION public.claim_schedule_dispatch(UUID, TIMESTAMPTZ, TIMESTAMPTZ)
  IS 'Atomically claims a schedule dispatch window and enqueues one queued task per (schedule_id, dispatch_window_start). Restricted to service_role. Idempotency via user_schedule_dispatches_idempotency. Caller must pass dispatch_window_start < dispatch_window_end and a start >= user_schedules.next_run.';
