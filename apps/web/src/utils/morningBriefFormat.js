...
function chunkSentences(sentences, maxPerChunk) {
    const chunks = [];
...
function normalizeSourceIdsFromMetadata(metadata) {
    if (!metadata || typeof metadata !== 'object')
        return [];
    const maybe = metadata;
...
function stripIdsFromNarrative(text) {
    const normalized = normalizeNewlines(text);
...
export function maskSourceId(id) {
    const cleaned = String(id || '').trim();
    if (cleaned.length <= 8)
        return cleaned;
    return `${cleaned.slice(0, 8)}...`;
}
export function formatMorningBriefNarrative(summaryText, metadata) {
...
//# sourceMappingURL=morningBriefFormat.js.map