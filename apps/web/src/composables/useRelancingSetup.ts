...
function isValidFutureDeadline(deadlineIso: string | null): boolean {
...
function normalizeDeadline(deadlineInput: string): string | null {
...
function collectMissingFields(projectName: string, members: string[], deadlineIso: string | null): string[] {
  const missing: string[] = [];
  if (projectName.trim().length === 0) missing.push('project_name');
  if (members.length < 1) missing.push('members');
  if (!isValidFutureDeadline(deadlineIso)) missing.push('deadline');
  return missing;
}

export function useRelancingSetup() {
  async function loadSnapshot(organizationId: string): Promise<RelancingSetupSnapshot> {
    ...
    const context = contextRows?.[0] as {
      ...
      setup_status: RelancingSetupStatus;
    } | undefined;
  ...
  async function saveSetup(input: SaveRelancingSetupInput): Promise<RelancingSetupSnapshot> {
    ...
    const setupStatus: RelancingSetupStatus = missingFields.length > 0 ? 'incomplete' : 'complete';
    ...
    if (members.length > 0) {
      const { error: memberUpsertError } = await supabase.from('project_member_assignments').upsert(
