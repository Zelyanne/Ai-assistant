                ...
                return {
                    ...
                    update: vi.fn((payload) => ({
                        eq: vi.fn(async (column, value) => {
                            if (column === 'project_context_id') {
                                members = members.map((row) => row.project_context_id === value ? { ...row, ...payload } : row);
                            }
                            return { error: null };
                        }),
                    })),
                    upsert: vi.fn(async (payload) => {
                        for (const row of payload) {
                            const existingIndex = members.findIndex((member) => member.project_context_id === row.project_context_id
...
//# sourceMappingURL=useRelancingSetup.spec.js.map