...
<template>
  <Card class="mb-3 hover:shadow-md transition-shadow border-l-4" :class="[`border-l-${categorySeverity}-500`]">
    <template #content>
      <div class="flex justify-between items-start">
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-1">
             <h4 class="font-bold text-lg text-gray-900 truncate">{{ email.subject }}</h4>
             <Tag :value="email.category || 'Uncategorized'" :severity="categorySeverity" class="text-xs" />
          </div>
          <p class="text-sm text-gray-600 line-clamp-2">{{ email.summary }}</p>
          
          <div v-if="isLimited" class="mt-2 text-xs text-orange-500 flex items-center gap-1">
              <i class="pi pi-bolt" />
        ...
        <div class="ml-4 flex flex-col items-end whitespace-nowrap">
            ...
            <div class="mt-2" v-if="email.priority_score">
                <span class="text-xs font-bold" :class="email.priority_score > 80 ? 'text-red-500' : 'text-gray-500'">
