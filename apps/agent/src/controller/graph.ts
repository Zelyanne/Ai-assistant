...
async function checkEmergencyBrake(state: AgentState): Promise<Partial<AgentState>> {
  ...
  try {
    const enabled = await SafetyControlsService.isEmergencyBrakeEnabled(task.organization_id);
    if (!enabled) return {};

    const summary = 'Emergency Brake engaged. Task paused.';
    const step = AuditLogger.createStep('Emergency Brake', 'Paused: Emergency Brake engaged', {
      confidence_score: 1,
    ...
    console.error(`[Graph][${task.id}] Emergency brake check failed: ${safeMessage}`);
    const step = AuditLogger.createStep('Emergency Brake', `Paused: brake check failed (${safeMessage})`, {
      confidence_score: 0,
...
async function initializeTask(state: AgentState, _config?: RunnableConfig): Promise<Partial<AgentState>> {
  ...
  try {
    ...
    const safeMessage = redactErrorMessage(err);
    console.error(`[Graph][${state.task.id}] Initialization failed: ${safeMessage}`);
    return { 
...
async function checkPerimeter(state: AgentState): Promise<Partial<AgentState>> {
  ...
  const requiredTier: AgencyTier = payloadWithOverride?.protocol_overridden_tier
    ...
    const errorStep = AuditLogger.createStep('Perimeter Check', `Check failed: ${safeMessage}`);
    return { 
...
async function executeProcessor(state: AgentState): Promise<Partial<AgentState>> {
  ...
  if ((typeof payloadConfidence === 'number' && payloadConfidence < CONFIDENCE_THRESHOLD) || payloadAmbiguity) {
    ...
    const trigger: EscalationTrigger = payloadAmbiguity ? 'ambiguity_detected' : 'low_confidence';

    const step = AuditLogger.createStep('Confidence Gate', `Escalated: ${reason}`, {
  ...
  try {
    ...
    if (state.task.domain_action === 'email.send' && safeMessage.startsWith('APPROVAL_')) {
      ...
      if (safeMessage.startsWith('APPROVER_MISMATCH')) {
        ...
        prompt = 'Connect Google Workspace and set an integration owner before sending.';
      } else if (safeMessage.startsWith('APPROVAL_DRAFT_INVALID')) {
        reason = 'Approved draft is missing required fields.';
        prompt = 'Fill required draft fields (To, Subject, Body), then approve again.';
      }

      const step = AuditLogger.createStep('Tool Execution', `Escalated: ${reason}`, {
        confidence_score: 0,
        confidence_threshold: CONFIDENCE_THRESHOLD,
        ambiguity_detected: true,
        escalation_trigger: 'approval_guardrail',
    ...
    const step = AuditLogger.createStep('Tool Execution', `Execution failed: ${safeMessage}`);
...
async function processEmailSend(state: AgentState) { return executeProcessor(state); }
...
function findFromHeader(metadata: unknown): string | null {
  ...
  for (let i = messages.length - 1; i >= 0; i -= 1) {
    ...
    for (const header of headers) {
      ...
      if (typeof name === 'string' && name.toLowerCase() === 'from' && typeof value === 'string') {
        return value;
      }
    }
  }
...
async function processThreadAction(state: AgentState): Promise<Partial<AgentState>> {
  if (state.error) return {};

  const task = state.task;
  const payload = (task.payload ?? {}) as Record<string, unknown>;

  const threadIdFromPayload = typeof payload.thread_id === 'string' ? payload.thread_id : null;
  ...
  const threadId = sourceType === 'thread' && sourceId ? sourceId : threadIdFromPayload;

  const guard = new PerimeterGuard();
  const topic = task.topic || 'General';

  const payloadAgencyTier = typeof payload.agency_tier === 'string' ? payload.agency_tier : null;
  const authorizedTier = (payloadAgencyTier === 'Public' || payloadAgencyTier === 'Controlled' || payloadAgencyTier === 'Restricted')
    ? payloadAgencyTier
    : 'Public';

  if (!threadId) {
    const reason = 'Missing thread identifier (payload.thread_id or payload.source_id when source_type=thread)';
    const escalationResult = buildEscalationPayload({
      reason,
    ...
    return {
      ...
      error: reason,
      trace: [AuditLogger.createStep('Thread Action', 'Escalated: missing thread id', {
        confidence_score: 0,
        confidence_threshold: CONFIDENCE_THRESHOLD,
        ambiguity_detected: true,
        escalation_trigger: 'ambiguity_detected',
  ...
  if (threadError || !thread) {
    ...
    return {
      ...
      error: reason,
      trace: [AuditLogger.createStep('Thread Load', `Escalated: ${reason}`, {
        confidence_score: 0,
        confidence_threshold: CONFIDENCE_THRESHOLD,
        ambiguity_detected: true,
        escalation_trigger: 'ambiguity_detected',
  ...
  const citations = [
    AuditLogger.createCitation('email', String(thread.external_id), 'Original Gmail thread', citationLink)
  ];
  ...
  const redactedSummaryJson = guard.redactPII(JSON.stringify(summaryJson ?? {}));
  ...
  try {
    const decisionResponse = await llm.generateStructured(prompt, ThreadActionDecisionSchema);
    decision = ThreadActionDecisionSchema.parse(decisionResponse.data);
  } catch (err: unknown) {
    const message = guard.redactPII(err instanceof Error ? err.message : String(err));
    ...
    return {
      ...
      error: reason,
      trace: [AuditLogger.createStep('Thread Action Decision', `Escalated: ${reason}`, {
        confidence_score: 0,
        confidence_threshold: CONFIDENCE_THRESHOLD,
        ambiguity_detected: true,
        escalation_trigger: 'ambiguity_detected',
  ...
  const decisionStep = AuditLogger.createStep('Thread Action Decision', `Planned action: ${decision.action}`, {
  ...
  if (authorizedTier === 'Controlled') {
    ...
    const draftBody = emailDraft?.body ?? 'Draft unavailable. Please write a response.';
    ...
    return {
      ...
      trace: [
        decisionStep,
        AuditLogger.createStep('Thread Action', 'Escalated with approval-ready draft for Controlled topic', {
          confidence_score: decision.confidence,
          confidence_threshold: CONFIDENCE_THRESHOLD,
          ambiguity_detected: decision.ambiguity_detected,
          escalation_trigger: 'approval_guardrail',
  ...
  if (decision.action === 'escalate' || decision.confidence < CONFIDENCE_THRESHOLD || decision.ambiguity_detected) {
    const reason = decision.action === 'escalate'
      ? (decision.escalation?.reason || 'Escalation requested by planner')
      : (decision.ambiguity_detected ? 'Ambiguity detected' : 'Low confidence');

    const promptText = decision.escalation?.prompt || 'Please review this thread and confirm next action.';
    const escalationTrigger: EscalationTrigger = decision.ambiguity_detected || decision.action === 'escalate'
      ? 'ambiguity_detected'
      : 'low_confidence';
    ...
    return {
      ...
      error: reason,
      trace: [decisionStep, AuditLogger.createStep('Thread Action', 'Escalated: planner confidence/ambiguity', {
        confidence_score: decision.confidence,
        confidence_threshold: CONFIDENCE_THRESHOLD,
        ambiguity_detected: decision.ambiguity_detected,
  ...
  try {
    if (decision.action === 'calendar.create') {
      const calendarProcessor = ProcessorRegistry.getProcessor('calendar.create');
      if (!calendarProcessor) throw new Error('calendar.create processor unavailable');
      ...
      const calendarTask: Task = {
        ...
        payload: {
          ...
          description: calendar.description ? guard.recoverPII(calendar.description) : undefined,
          location: calendar.location ? guard.recoverPII(calendar.location) : undefined,
        }
      } as Task;

      await calendarProcessor.process(calendarTask);

      const step = AuditLogger.createStep('Tool Execution', 'Executed calendar.create', {
    ...
    const recipient = fromHeader ? extractEmailAddress(fromHeader) : null;
    if (!recipient) {
      const reason = 'Could not determine recipient address from thread metadata';
      const escalationResult = buildEscalationPayload({
        reason,
      ...
      return {
        ...
        error: reason,
        trace: [decisionStep, AuditLogger.createStep('Thread Action', 'Escalated: missing recipient', {
          confidence_score: 0,
          confidence_threshold: CONFIDENCE_THRESHOLD,
          ambiguity_detected: true,
          escalation_trigger: 'ambiguity_detected',
    ...
    const toolStep = AuditLogger.createStep('Tool Execution', 'Executed email.draft', {
    ...
    return {
      ...
      error: reason,
      trace: [decisionStep, AuditLogger.createStep('Tool Execution', `Escalated: ${reason}`, {
        confidence_score: 0,
        confidence_threshold: CONFIDENCE_THRESHOLD,
        ambiguity_detected: true,
        escalation_trigger: 'approval_guardrail',
...
async function finalizeTask(state: AgentState, _config?: RunnableConfig): Promise<Partial<AgentState>> {
  ...
  try {
    ...
    const safeMessage = redactErrorMessage(err);
    console.error(`[Graph][${state.task.id}] Finalization failed: ${safeMessage}`);
  }
...
function routeAfterReasoning(state: AgentState) {
  ...
  const ambiguity = typeof lastStep?.ambiguity_detected === 'boolean' ? lastStep.ambiguity_detected : !hasConfidence;
