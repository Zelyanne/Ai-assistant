...
function buildSlidesHandoff(invocations: ToolInvocationRecord[], finalMessage: string): SpecialistNodeResult {
  const lastInvocation = invocations[invocations.length - 1];
  const lastResult = lastInvocation?.result;
  const resourceId = findFirstString(lastResult, ['presentation_id', 'id']);
  const resourceUrl = findFirstUrl(lastResult);
  const summary = asString(asRecord(lastResult).summary) ?? finalMessage ?? 'Slides specialist completed.';
  const nextWorkerNote = resourceUrl ? `Presentation ready${resourceId ? ` (${resourceId})` : ''}: ${resourceUrl}` : summary;
  return {
    summary,
    nextWorkerNote,
    toolName: lastInvocation?.toolName,
    output: { summary, handoff_content: nextWorkerNote, presentation_id: resourceId ?? undefined, presentation_url: resourceUrl ?? undefined, tool_name: lastInvocation?.toolName },
  };
}

export async function slidesAgentNode(context: SpecialistNodeContext): Promise<SpecialistNodeResult> {
  const invocations: ToolInvocationRecord[] = [];
  const mcpTools = await getSpecialistMcpTools(context.task.organization_id, 'slides', {
    userId: context.task.user_id,
  });

  if (mcpTools.length === 0) throw new Error('No Slides MCP tools available for this organization.');
  ...
  try {
    const agent = createAgent({
      model: llm,
      tools: trackedTools,
      systemPrompt: getSpecialistPrompt('slides'),
    ...
    if (invocations.length === 0) throw new Error('Slides specialist did not execute any tool.');

    tracingService.handleSuccess();
    await tracingService.flush();
    return buildSlidesHandoff(invocations, finalMessage);
  } catch (error) {
