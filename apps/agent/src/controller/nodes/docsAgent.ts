...
function buildDocsHandoff(context: SpecialistNodeContext, invocations: ToolInvocationRecord[], finalMessage: string): SpecialistNodeResult {
  const lastInvocation = invocations[invocations.length - 1];
  const lastResult = lastInvocation?.result;
  const documentId = findFirstString(lastResult, ['documentId', 'document_id', 'id']);
  const documentUrl = findFirstUrl(lastResult);
  const title = asString(asRecord(lastResult).title) ?? asString(asRecord(context.step.input).title) ?? 'Google Doc';
  const summary = asString(asRecord(lastResult).summary) ?? finalMessage ?? `Docs agent created ${title}.`;
  const nextWorkerNote = documentUrl ? `Google Doc ready: ${title} (${documentUrl}).` : `Google Doc ready: ${title}.`;
  return {
    summary,
    nextWorkerNote,
    toolName: lastInvocation?.toolName,
    output: { summary, handoff_content: nextWorkerNote, document_id: documentId ?? undefined, document_url: documentUrl ?? undefined, document_title: title, tool_name: lastInvocation?.toolName },
  };
}

export async function docsAgentNode(context: SpecialistNodeContext): Promise<SpecialistNodeResult> {
  const invocations: ToolInvocationRecord[] = [];
  const mcpTools = await getSpecialistMcpTools(context.task.organization_id, 'docs', {
    userId: context.task.user_id,
  });

  if (mcpTools.length === 0) throw new Error('No Docs MCP tools available for this organization.');
  ...
  try {
    const agent = createAgent({
      model: llm,
      tools: trackedTools,
      systemPrompt: getSpecialistPrompt('docs'),
    ...
    if (invocations.length === 0) throw new Error('Docs specialist did not execute any tool.');

    tracingService.handleSuccess();
    await tracingService.flush();
    return buildDocsHandoff(context, invocations, finalMessage);
  } catch (error) {
