...
const envSchema = z.object({
  ...
  SEARXNG_MCP_URL: z.string().url().optional().transform((val) => val?.trim()),
  SEARXNG_URL: z.string().url().optional().transform((val) => val?.trim()),
  SEARXNG_MCP_USERNAME: z.string().min(1).optional().transform((val) => val?.trim()),
  SEARXNG_MCP_PASSWORD: z.string().min(1).optional().transform((val) => val?.trim()),
  ...
  TELEGRAM_WEBHOOK_URL: z.string().url().optional().transform((val) => val?.trim()),
